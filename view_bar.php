...
$viewBar = $_GET['bid'];
$sql = "SELECT * FROM kroeg WHERE kroegcode='$viewBar'";
$query = mysqli_query($conn, $sql);
$row = mysqli_fetch_array($query);
$barName = $row['naam'];
$barAddress = $row['adres'];
$barPlace = $row['plaats'];
$barAuthor = $row['gebruikerscode'];

//Haal informatie uit bier via schenkt
$sql3 = "SELECT bier.biercode as bid, naam FROM bier, schenkt WHERE kroegcode = '$viewBar' AND schenkt.biercode = bier.biercode";
$query3 = mysqli_query($conn, $sql3);

//Haal gebruikers informatie uit gebruiker
$sql4 = "SELECT eerstenaam, laatstenaam FROM gebruiker WHERE gebruikerscode = '$barAuthor'";
...
if(isset($_POST['post-reaction'])) {
  ...
  if($error == 0) {
    ...
    VALUES ('$viewBar', '$text', '$choice', '$rating', '$date', 'kroeg', '$userSelf')";
    $res = mysqli_query($conn, $sql);
  }
}
...
<head>
  ...
  <title>BierKiezer - <?php echo uppercase($barName); ?></title>
...
<div class="container">
    ...
    <div class="col-md-6">
      <div class="row">
        <table class="table-striped col-md-12">
          <tr>
            <th colspan="2">
              <h3 class="col-sm-offset-1 col-sm-10"><?php echo $barName; ?></h3>
              <?php
                if (isset($_SESSION['admin'])) {
                  if($_SESSION['admin'] == 2 || $userSelf === $barAuthor) { ?>
                    <a href="edit_bar.php?bid=<?php echo $viewBar; ?>" class="editBtn">
          ...
          <tr>
            ...
            <td><a href="view_profile.php?user=<?php echo $barAuthor; ?>"><h4><?php echo $wholeName; ?></h4></a></td>
          </tr>
        </table>
      </div>
      <hr>
        ...
        <table class="table-striped col-md-12">
          ...
          if(mysqli_num_rows($query3) > 0) {
            while($row3 = mysqli_fetch_assoc($query3)) {
              echo "<tr><td><a href='view_beer.php?bid=".$row3['bid']."'><h5>".$row3['naam']."</h5></a></td></tr>";
            }
          } else {
            echo "<tr class='warning'><td><p>Deze kroeg schenkt geen bier van onze database.</p></td></tr>";
          }
          ?>
        </table>
      </div>
  ...
  <div class="row">
    <table class="table-striped col-md-offset-1 col-md-8">
      ...
      <?php
      $sql = "SELECT * FROM beoordeling WHERE productcode = '$viewBar' AND type = 'kroeg' ORDER BY beoordelingscode DESC";
      $res = mysqli_query($conn, $sql);
        ...
        if (isset($_SESSION['eerstenaam'])) {
          ...
          <?php } ?>
          <form action="view_bar.php?bid=<?php echo $viewBar; ?>" method="post">
            <tr>
