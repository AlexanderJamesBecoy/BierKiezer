...
$viewBrew = $_GET['bid'];
$sql = "SELECT * FROM brouwer WHERE brouwcode='$viewBrew'";
...
//Haal informatie uit bier
$sql3 = "SELECT * FROM bier WHERE brouwcode = '$viewBrew'";
$query3 = mysqli_query($conn, $sql3);

//Haal gebruikers informatie uit gebruiker
$sql4 = "SELECT eerstenaam, laatstenaam FROM gebruiker WHERE gebruikerscode = '$brewAuthor'";
...
if(isset($_POST['post-reaction'])) {
  ...
  if($error == 0) {
    ...
    VALUES ('$viewBrew', '$text', '$choice', '$rating', '$date', 'brouwer', '$userSelf')";
    $res = mysqli_query($conn, $sql);
  }
}
...
<head>
  ...
  <title>BierKiezer - <?php echo uppercase($brewName); ?></title>
...
<div class="container">
    ...
    <div class="col-md-6">
      <div class="row">
        <table class="table-striped col-md-12">
          <tr>
            <th colspan="2">
              <h3 class="col-sm-offset-1 col-sm-10"><?php echo $brewName; ?></h3>
              <?php
                if (isset($_SESSION['admin'])) {
                  if($_SESSION['admin'] == 2 || $userSelf === $brewAuthor) { ?>
                    <a href="edit_brewery.php?bid=<?php echo $viewBrew; ?>" class="editBtn">
          ...
          <tr>
            ...
            <td><a href="view_profile.php?user=<?php echo $brewAuthor; ?>"><h4><?php echo $wholeName; ?></h4></a></td>
          </tr>
        </table>
      </div>
      <hr>
        ...
        <table class="table-striped col-md-12">
          ...
          if(mysqli_num_rows($query3) > 0) {
            while($row3 = mysqli_fetch_assoc($query3)) {
              echo "<tr><td><a href='view_beer.php?bid=".$row3['biercode']."'><h5>".$row3['naam']."</h5></a></td></tr>";
            }
          } else {
            echo "<tr class='warning'><td><p>Deze brouwer brouwt niets van onze database.</p></td></tr>";
          }
          ?>
        </table>
      </div>
  ...
  <div class="row">
    <table class="table-striped col-md-offset-1 col-md-8">
      ...
      <?php
      $sql = "SELECT * FROM beoordeling WHERE productcode = '$viewBrew' AND type = 'brouwer' ORDER BY beoordelingscode DESC";
      $res = mysqli_query($conn, $sql);
        ...
        if (isset($_SESSION['eerstenaam'])) {
          ...
          <?php } ?>
          <form action="view_brewery.php?bid=<?php echo $viewBrew; ?>" method="post">
            <tr>
