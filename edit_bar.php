...
$barCode = $_GET['bid'];
$sql = "SELECT * FROM kroeg WHERE kroegcode = '$barCode'";
$res = mysqli_query($conn, $sql);
$row = mysqli_fetch_array($res);
$barName = $row['naam'];
$barAddress = $row['adres'];
$barPlace = $row['plaats'];
...
if(isset($_POST['submit'])) {
	if($_POST['naam'] === NULL || $_POST['naam'] === "") {
		...
		$newBarName = strtoupper($_POST['naam']);
	}

	if($_POST['adres'] === NULL || $_POST['adres'] === "") {
		$newBarAddress = $barType;
	} else {
		$newBarAddress = strtoupper($_POST['adres']);
	}

	if($_POST['plaats'] === NULL || $_POST['plaats'] === "") {
		$newBarPlace = $barPlace;
	} else {
		$newBarPlace = strtoupper($_POST['plaats']);
	}

	$sql = "UPDATE kroeg SET naam = '$newBarName', adres = '$newBarAddress', plaats = '$newBarPlace' WHERE kroegcode = '$barCode'";
	$res = mysqli_query($conn, $sql);
	header("Location: view_bar.php?bid=$barCode");
}
...
<head>
  ...
  <title>BierKiezer - <?php echo uppercase($barName); ?> - Wijzigen</title>
...
<div class="container">
    ...
    <div class="col-md-6">
        ...
        <table class="table-striped col-md-12">
					<tr>
						<th colspan="2"><h3 class="col-sm-offset-1 col-sm-10"><?php echo $barName; ?> - Originele Gegevens</h3></th>
					</tr>
          <tr>
            <td><h4 class="col-sm-offset-2">Naam</h4></td>
						<td><h4><?php echo uppercase($barName); ?></h4></td>
          </tr>
          <tr>
            <td><h4 class="col-sm-offset-2">Adres</h4></td>
            <td><h4><?php echo uppercase($barAddress); ?></h4></td>
          </tr>
          <tr>
            <td><h4 class="col-sm-offset-2">Plaats</h4></td>
            <td><h4><?php echo uppercase($barPlace); ?></h4></td>
          </tr>
        </table>
      </div>
      <div class="row">
				<form action="edit_bar.php?bid=<?php echo $barCode; ?>" method="post">
	        <table class="table-striped col-md-12">
						<tr>
							<th colspan="2"><h3 class="col-sm-offset-1 col-sm-10"><?php echo $barName; ?> - Wijzigen</h3></th>
						</tr>
	          <tr>
	            <td><h4 class="col-sm-offset-2">Naam</h4></td>
							<td><input type="text" class="form-control" name="naam" value="<?php echo uppercase($barName); ?>"></td>
	          </tr>
	          <tr>
	            <td><h4 class="col-sm-offset-2">Adres</h4></td>
	            <td><input type="text" class="form-control" name="adres" value="<?php echo uppercase($barAddress); ?>"></td>
	          </tr>
	          <tr>
	            <td><h4 class="col-sm-offset-2">Plaats</h4></td>
	            <td><input type="text" class="form-control" name="plaats" value="<?php echo uppercase($barPlace); ?>"></td>
	          </tr>
						<tr>
